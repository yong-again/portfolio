"""
Side Defect Detection Service

측면(Side/Corner) 영역의 결함 검출 서비스입니다.
회사 코드의 SidePipeline을 참고하여 리팩토링되었습니다.
"""
...
class SideService:
    ...
    def _load_models(self):
        ...
        # Defect Segmentation 모델 (측면 결함 검출)
    ...
    def _postprocess_mask(
        ...
        postprocessed = self.postprocessor.postprocess(defect_mask, section='side')
        
        return postprocessed
    
    def _check_corner_b_pixel_count(
        self,
        mask: np.ndarray
    ) -> int:
        """
        측면 영역의 B 등급 픽셀 수를 확인합니다.
    ...
    def infer(
        ...
        Returns:
            검출 결과 딕셔너리
        """
        side_config = self.config['defect_segmentation']['side']
        
        # 설정 파라미터 가져오기
        scaler = side_config['preprocessing'].get('scaler', ['normalize'])
        img_size = side_config['input_size']
        pixel_count_thres = side_config.get('postprocessing', {}).get('pixel_count_threshold', 0)
        ...
        # 4. 측면 영역 특화 후처리
        # B 픽셀 수 검사
        mask_for_check = post_images[0] if len(post_images.shape) == 3 else post_images
        pixel_count_corner = self._check_corner_b_pixel_count(mask_for_check)
        
        # B 픽셀 수가 임계값 이상이면 C 등급으로 변경 (3)
        if pixel_count_corner >= pixel_count_thres:
            post_images[post_images == 2] = 3
        ...
        return {
            'defect_mask': post_images.tolist() if isinstance(post_images, np.ndarray) else post_images,
            'analysis': analysis_result,
            'grade': grade_result,
            'top_defects': top_defects,
            'b_pixel_count': int(pixel_count_corner),
            'pixel_count_threshold_exceeded': pixel_count_corner >= pixel_count_thres
        }
