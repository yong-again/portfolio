"""
Data Cleaning Script for CrowdHuman YOLO Dataset

Based on EDA findings, this script performs:
1. Remove duplicate images
2. Filter invalid bounding boxes
3. Remove low-quality images (blur threshold)
4. Clip bounding boxes to image boundaries
5. Remove outlier head boxes (size-based outlier detection)
6. Remove empty annotations after cleaning
...
def clip_bbox(x: float, y: float, w: float, h: float, img_w: int, img_h: int) -> Tuple[float, float, float, float]:
    ...
    # Adjust width and height if box extends beyond boundaries
    if x + w > img_w:
        w = img_w - x
    if y + h > img_h:
        h = img_h - y
...
def is_head_box_outlier(w: float, h: float, area: float, aspect_ratio: float,
    ...
    """Check if a head box is an outlier based on size statistics."""
...
def collect_head_box_statistics(root_dir: Path, splits: List[str]) -> Dict[str, np.ndarray]:
    """Collect statistics for head boxes (class_id = 0) across all splits."""
        ...
        for img_path in tqdm(image_files, desc=f"Collecting stats from {split}", leave=False):
            ...
            try:
                ...
                for line in lines:
                    ...
                    class_id = int(parts[0])
                    # Only collect stats for head boxes (class_id = 0)
                    if class_id != 0:
                        continue
                    
                    # Convert YOLO to xywh
...
def clean_label_file(label_path: Path, image_path: Path, min_size: int = 2,
    ...
    Returns:
        Tuple of (valid_lines, removed_count, removed_outliers_count)
    """
    if not label_path.exists():
        return [], 0, 0
    ...
    try:
        ...
        for line in lines:
            ...
            # Check for head box outliers (class_id = 0)
...
def clean_dataset(
    ...
    """
    Clean the YOLO dataset.
    
    Args:
        root_dir: Root directory of YOLO dataset (contains images/ and labels/)
        output_dir: Output directory for cleaned dataset
        min_blur_score: Minimum blur score threshold (lower = blurrier)
        min_bbox_size: Minimum bounding box size in pixels
        remove_duplicates: Whether to remove duplicate images
        remove_head_outliers: Whether to remove outlier head boxes
        outlier_iqr_multiplier: IQR multiplier for outlier detection (default: 1.5)
        splits: List of splits to clean (default: ['train', 'val', 'test'])
    """
    if splits is None:
    ...
    # Collect head box statistics for outlier detection
    ...
    for split in splits:
        ...
        for img_path in tqdm(image_files, desc=f"Cleaning {split}"):
            ...
            # Copy image and write cleaned label
